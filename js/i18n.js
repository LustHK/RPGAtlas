...
(() => {
  ...
  async function loadLocale(localeId) {
    ...
    try {
      const res = await fetch("locales/" + encodeURIComponent(localeId) + ".json");
      if (res.ok) {
  ...
  function normalizeEditorLocale(locale) {
    const language = String(locale || "").trim().toLowerCase().replace("_", "-").split("-")[0];
    return Object.prototype.hasOwnProperty.call(LOCALE_META, language) ? language : "en";
  }
  ...
  async function createI18n(options = {}) {
    ...
    return {
      ...
      locales: () => Object.entries(LOCALE_META).map(([id, label]) => ({ id, label })),
      localizeStatic,
      setLocale,
      t,
    };
  ...
  window.RPGAtlasI18n = {
    create: createI18n,
    normalizeLocale: normalizeEditorLocale,
